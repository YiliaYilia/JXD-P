...
public class DateUtil {
	...
	public static long getMinuteDistanceNow(Date endTime){
		SimpleDateFormat d = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss");// 格式化时间  
        String nowtime = d.format(new Date());// 按以上格式 将当前时间转换成字符串  
        long result=0;
        try {  
            result = (d.parse(d.format(endTime)).getTime() - d.parse(nowtime)  
	...
	public static void main(String[] args) {
		Date endTime =DateUtil.addMinute(new Date(), 45);
		long payCloseTime=DateUtil.getMinuteDistanceNow(endTime);
		System.out.println(payCloseTime);
	}
}