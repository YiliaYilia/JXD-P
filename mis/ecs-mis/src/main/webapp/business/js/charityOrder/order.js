...
Order.exp =  {
		...
		handler : function() {
			...
			window.location.href='../business/expCharityOrder?ordercode='+ordercode+'&paystatus='+paystatus+'&recipientname='+recipientname+'&productname='
								+productname+'&starttime='+insuretimebegin+'&endtime='+insuretimeend;
		}
};
/**
 * 工具栏
...
Order.prototype.createDataGrid = function(){
	...
	$('#orderData').datagrid({
		...
		url : that.ctx+'/business/findCharityOrderData',
		fit : true,
		pagination:true,
		rownumbers:true,
		pageSize:15,
...
function dictTrans1(dictCode,dictType){
	if(result==''){
		result = [{"dictdatacode":"0","dictdataname":"未支付","dicttypecode":"PAYSTATUS"},
		          ...
		          {"dictdatacode":"3","dictdataname":"本地成功，支付接口失败","dicttypecode":"STATUS"},
		          ];
	}
	result = eval(result);
