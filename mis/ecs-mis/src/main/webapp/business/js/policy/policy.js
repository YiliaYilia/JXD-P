...
Policy.exp =  {
		...
		//	window.location.href='../business/expPolicy'
			...
			var agentcode = $("#agentcode-search").val().replace(/\s+/g,"");
			var brno = $("#brno-search").val().replace(/\s+/g,"");
			var deptid = $("#deptid-search").val();
			var userid = $("#userid-search").val().replace(/\s+/g,"");
			window.location.href='../business/expPolicy?policycode='+policycode+'&productname='+productname+'&applicantname='+applicantname+'&status='
									+status+'&starttime='+insuretimebegin+'&endtime='+insuretimeend+'&agentcode='+agentcode+'&brno='+brno+'&deptid='
...
Policy.prototype.loadData = function(){
	var userid = $("#userid-search").val().replace(/\s+/g,"");
	$('#policyData').datagrid('options').url = this.ctx+"/business/findPolicyData?userid="+userid;
}
