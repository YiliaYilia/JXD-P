...
import cn.com.sinosoft.web.domain.MyX509TrustManager;
...
public class CommonUtil {
    ...
    /**
     * 获取接口访问凭证
     * 此方法只允许定时任务进行调用，其他用到token值需在内存中获取
     * @param appid 凭证
     * @param appsecret 密钥
     * @return
     */
...
//        if(token == null){
	        ...
	        if (null != jsonObject) {
	            try {
	                token = Token.getInstance(jsonObject.getString("access_token"),jsonObject.getInt("expires_in"));
	                log.info("获取access_token成功: "+token.getAccessToken());
