...
String url = "http://testweixin.oldmutual-guodian.com/online/action/KTresult";
...
<html>
	...
	<body>
		<div class="main_wrap">
			<img src="<%=request.getContextPath() %>/resources/image/calculator/ktjs.jpg" class="big_img"/>
			<span class="title">康泰金生重大疾病保险计划</span>
...
<!-- 					</tr> -->
					<tr>
						<td colspan="2">
							<a onclick="window.location.href='<%=request.getContextPath()%>/action/KTxj';">
								<p class="detail">康泰金生重大疾病保险现金价值表详情</p>
							</a>
						</td>
					</tr>
					<tr>
			...
			<div class="content">
				...
				<h5>轻症疾病保险金（10种）</h5>
				<p class="con_con">
					&#10148 180天后初次确诊轻症疾病，按<span style="color:blue;"><%=session.getAttribute("coverage")%>万</span>的25%给付轻症疾病保险金，本项责任终止，重大疾病保险责任继续有效
				</p>
				<h5>重大疾病保险金（40种）</h5>
				<p class="con_con">
					&#10148 180天内初次确诊，按累计保费120%给付重大疾病保险金 ，保险合同终止<br/>
					&#10148 180天后初次确诊，按<span style="color:blue;"><%=session.getAttribute("coverage")%>万</span>给付重大疾病保险金 ，保险合同终止<br/>
				</p>
				<h5>身故保险金</h5>
				<p class="con_con">
					&#10148 180日内因病：给付保险费的120%，保险合同终止。<br/>
					&#10148  因意外或180日后身故：<br/>
					&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;未满18周岁，给付保险费的120%，保险合同终止<br/>
					&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;年满18周岁，给付<span style="color:blue;"><%=session.getAttribute("coverage")%>万</span>，保险合同终止
				</p>
				<p style="font-size:60%;">
					保险责任、责任免除及犹豫期等内容详见瑞泰康泰金生重大疾病保险合同条款<br/>
					未成年人身故保险金的给付按照国务院保险监督管理机构规定的限额执行
				</p>
			</div>
			<div class="content" onclick="window.location.href='<%=request.getContextPath()%>/resources/pdf/calculator/kt.pdf';">
			...
			<div class="content" style="margin-bottom: 20px; height:2rem;">
				<p class="con_title imp_c" onclick="share()" style="font-weight:normal;font-size:100%;">分享到朋友圈</p>
			</div>
		</div>
		<div class="layer"></div>
...
function trial(){
	...
	if(valueage > 106 || isNaN(valueage)){
		...
		$.ajax({
		   type:"POST",
		   async: false,
		   url:"<%=request.getContextPath()%>/action/KTtrial",
		   data:{
			   "valueage":valueage
		   },
		   dataType:"json",
...
wx.ready(function () {
	...
	wx.onMenuShareAppMessage({
		title: "康泰金生重大疾病保险!", // 分享标题
	    desc: "康泰金生重大疾病保险!", // 分享描述
// 	    link:"http://testweixin.oldmutual-guodian.com/online/action/KTCalculator", //分享链接
	    imgUrl:"" // 分享图标
	});
