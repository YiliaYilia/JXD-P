...
String url = "http://weixin.oldmutual-guodian.com/onlineApp/msl/sales/WJXHomePage?"+request.getQueryString();
...
<html>
  ...
  <body onload="backshow()">
  	...
  	<section class="product_wrap">
  		...
  		<%-- <img  src="<%=request.getContextPath() %>/resources/image/wjx/pro01.jpg" alt="产品图" class="product_img"/> --%>
  		<div class="clear"></div>
  		<form action="<%=request.getContextPath()%>/msl/sales/wjxFill" method="post" id="baseForm" name="form1">
	  		<table class="info_table">
	  			<tr>
	  				<td class="txt14" style="text-indent: 5%"><span class="must">*</span>身份证号:</td>
	  				<td><input id="identifynumber" name="identifynumber" type="text" class="txt14" value='' onChange="changeIdNo();"/></td>
	  			</tr>
	  			<tr>
	  				<td class="txt14" style="text-indent: 5%"><span class="must">*</span>投保份数:</td>
	  				<td><select id="count" name="policy.contnumber" class="txt14" onChange="changeIdNo();">
	  			...
	  			<tr>
	  				<td class="txt14" style="color: red; font-size:0.8em;word-break:break-all;" colspan="3">孕妇、家庭主妇、离退休、待业人员限投1份</td>
	  			</tr>
	  			<tr>
	  				<td class="txt14">基本保险金额:</td><td class="txt14"><span id="coverage">100000.00</span>元</td>
	  			</tr>
	  			<tr>
	  				<td class="txt14">保障期限:</td><td class="txt14">30年</td>
	  			</tr>
	  			<tr>
	  				<td class="txt14">交费期限:</td><td class="txt14">10年</td>
	  			</tr>
	  			<tr>
		...
		<section class="info_hide">
			<img src="<%=request.getContextPath() %>/resources/image/wjx/pro02.jpg" alt="产品图" class="product_img"/>
			<div class="clear"></div>
		</section>
		<section class="info" >
		...
		<section class="info_hide txt14">
			<%-- <a href="<%=request.getContextPath()%>/global/global/pdf/wjx_article/web/viewer.html">瑞泰稳驾行两全保险合同条款</a>
			<a href="<%=request.getContextPath()%>/global/global/pdf/wjx_rate_tab/web/viewer.html">瑞泰稳驾行两全保险产品费率表</a> --%>
			<a href="../../resources/pdf/wjx/wjx_article.pdf">瑞泰稳驾行两全保险合同条款</a>
			<a href="../../resources/pdf/wjx/wjx_rate_tab.pdf">瑞泰稳驾行两全保险产品费率表</a>
		</section>
  	</section>
  	<div class="footer">
...
wx.ready(function () {
	...
	if(brNo==null||brNo=="A0000"||brNo.length==0){
		linkUrl="https://open.weixin.qq.com/connect/oauth2/authorize?appid=wx5be4b58fee2539db&redirect_uri=http://weixin.oldmutual-guodian.com/onlineApp/msl/sales/WJXHomePage&response_type=code&scope=snsapi_base&state=STATE#wechat_redirect";
	}else{
		linkUrl="https://open.weixin.qq.com/connect/oauth2/authorize?appid=wx5be4b58fee2539db&redirect_uri=http://weixin.oldmutual-guodian.com/onlineApp/msl/sales/WJXHomePage?brNo=<%=brNo%>&response_type=code&scope=snsapi_base&state=STATE#wechat_redirect";
	}
	wx.showOptionMenu();
	var shareData = {
	        title: '百万驾乘意外', // 分享标题
  		...
  		var idno = document.getElementById("identifynumber").value;
  		 ...
  		 if(birthday!=""&&birthday!=null&&count!=null&&IdentityCodeValid(idno,true)){
  			$.ajax({
				   type:"POST",
				   async: false,
				   url:"<%=request.getContextPath()%>/msl/sales/WJXPolicyCalculate",
				   data:{
	...
	function getSex(value){
  	...
  	function dosubmit(){
  		...
  		if(changeIdNo()==false){
			  ...
			  document.getElementById("applicantInsured.identifynumber").value = idno;//保费
			  $("#baseForm").submit();
		  }
  	}
