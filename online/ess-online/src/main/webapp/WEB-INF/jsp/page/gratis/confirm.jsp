...
<html>
  ...
  <body onload="yuedu()">
  	...
  	<div class="policy_holder" style="display: none;"></div>
  	<div class="center_fill">
  	<div class="choose">
  	<table  class="msg">
  		<tr>
  			<td style="text-align:left;display: none;">推荐人编码</td>
  			<td style="float:left;width:100%;display: none;">${policy.agentcode}</td>
  		</tr>
  	</table>
  	...
  	<table class="agree">
  		<tr>
  			...
  			<td width=90%;>
  			我已阅读<a href="<%=request.getContextPath()%>/global/global/pdf/know_t/web/viewer.html">《投保须知及客户声明》、</a>
  			<a href="<%=request.getContextPath()%>/global/global/pdf/clause_t/web/viewer.html">《瑞泰天天保公共交通工具意外伤害保险合同条款》</a>
  			理解并同意其全部内容</td>
  		</tr>
  	</table>
  	<div id="toInsureSubmit" onclick="goApply();" class="notice_btn">下一步：确认投保</div>
...
function goApply(){
	...
	if(flag){
		...
		$.ajax({
			type:"POST",
			url:contentRootPath+"/msl/sales/insureGratisPolicySubmit",
			data:{
			...
			success:function(data){
				 if(data.resultSuccess){
					modal_loading('hide');
					document.getElementById("isSure").checked=false;
					window.location.href =contentRootPath+'/msl/sales/insureGratisPolicyPromise';
