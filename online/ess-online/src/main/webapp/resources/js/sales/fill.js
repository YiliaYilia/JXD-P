...
//	});
	...
	$.ajax({
		type:"POST",
		url:contentRootPath+"/sales/calculate",
		data:{
...
function toConfirm(){
	...
	$('#fillForm').validSubmit(function(valid){
		...
		$.ajax({
			type:"POST",
			url:contentRootPath+"/sales/insureInput",
			data:{
				"policy.policytermtype":					policytermtype,
			...
			success:function(data){
				 if(data.resultSuccess){
					 modal_loading('hide');
					 window.location.href =contentRootPath+'/sales/insureConfirm';
...
function echoCustomer(callback){
	if(useragent.isWechat){
		...
		$.ajax({
		   type: "POST",
		   url: contentRootPath+"/sales/echoCustomer",
