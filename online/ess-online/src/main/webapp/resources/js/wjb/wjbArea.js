...
where[15] = new comefrom("江西", "南昌|景德镇|九江|鹰潭|萍乡|新馀|赣州|吉安|宜春|抚州|上饶");
...
where1[0] = new comefrom("省/市","市/区");
where1[1] = new comefrom("北京市","北京市");
where1[2] = new comefrom("上海市","上海市");
where1[3] = new comefrom("广东省","广州市|深圳市");
where1[4] = new comefrom("江苏省","南京市|镇江市|苏州市|南通市|扬州市|盐城市|徐州市|连云港市|常州市|无锡市|宿迁市|泰州市|淮安市");
where1[5] = new comefrom("浙江省","杭州市|温州市|嘉兴市|湖州市|绍兴市|金华市|衢州市|舟山市|台州市|丽水市");
where1[6] = new comefrom("重庆市","重庆市");
where1[7] = new comefrom("陕西省","西安市|宝鸡市|咸阳市|铜川市|渭南市|延安市|榆林市|汉中市|安康市|商洛市");
where1[8] = new comefrom("湖北省","武汉市|宜昌市|荆州市|襄樊市|黄石市|荆门市|黄冈市|十堰市|潜江市|天门市|仙桃市|随州市|咸宁市|孝感市|鄂州市|恩施土家族苗族自治州");
function select() {
	with (document.creator.province) {
		var loca2 = options[selectedIndex].value;
	}
	for (i = 0; i < where1.length; i++) {
		if (where1[i].loca == loca2) {
	...
	if(loca2=="省/市"||loca4=="市/区"){
		loca2="";loca4="";
	}
	$("#address").html(loca2 + loca4);

	
	
	with (document.creator.bankProvinceAddress) {
		var loca2 = options[selectedIndex].value;
	}
	for (i = 0; i < where.length; i++) {
		if (where[i].loca == loca2) {
			...
			for (j = 0; j < loca3.length; j++) {
				with (document.creator.bankCityAddress) {
					length = loca3.length;
					options[j].text = loca3[j];
					options[j].value = loca3[j];
					var loca4 = options[selectedIndex].value;
...
function init() {
	with (document.creator.province) {
		...
		options[selectedIndex].text = where1[0].loca;
		options[selectedIndex].value = where1[0].loca;
	}
	with (document.creator.city) {
		loca3 = (where1[0].locacity).split("|");
		length = loca3.length;
		for (l = 0; l < length; l++) {
	...
	var check = document.getElementById("commonApplicant.flag").checked;
	if(check){
		$(".checked_img").show();
	}
	with (document.creator.bankProvinceAddress) {
...
function BankBranch(){
	with (document.creator.payBankProvinceAddress) {
		var loca2 = options[selectedIndex].value;
	}
	for (i = 0; i < where.length; i++) {
		if (where[i].loca == loca2) {
			loca3 = (where[i].locacity).split("|");
			for (j = 0; j < loca3.length; j++) {
				with (document.creator.paybankCityAddress) {
					length = loca3.length;
					options[j].text = loca3[j];
					options[j].value = loca3[j];
					var loca4 = options[selectedIndex].value;
	...
	$("#payBankProvince").html(loca2);
	$("#paybankCity").html(loca4);
}
function init2(){
	with (document.creator.payBankProvinceAddress) {
		length = where.length;
