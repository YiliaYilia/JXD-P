...
public class LongPolicyUtil {
	
	public static Logger logger = LoggerFactory.getLogger(LongPolicyUtil.class);
	 ...
	 * @return
	 */
	public static String calculate(String birthday,String sex,String amnt,String url){
		String TransrNo = getTransrNo();
		//拼报文
		LCBnfs lcbnfs = new LCBnfs("0",null);
		Accounts acconuts = new Accounts("1");
		Risk risk = new Risk("HEALTHGD","HEALTHGD","017","1",amnt,"","20150506","","0.00","","0","",null,"","","1","0","","","","","4","1","","","","","","","","",acconuts,"N","N","Y","N",lcbnfs);
		Risks risks = new Risks(1,risk);
		LCInsured lCInsured = new LCInsured("","林四",sex,birthday,"O","999999","","","","",null,"","","浙江省（市、区）杭州市西湖区县（区）竞舟路西鉴枫景公","310013","","","","","N","CH","CN","ZJ","HZ",null,risks);
		LCAppnt lcAppnt = new LCAppnt("","林三","M","19831029","O","999888","20100501","20300501","","","","13957171471","浙江省（市、区）杭州市西湖区县（区）竞舟路西鉴枫景公寓","310013","","","linlei101291@sina.com","","120000.00","240000.00","0","CN","ZJ","HZ","PA",null);
		LCInsureds lcInsureds = new LCInsureds("1",lCInsured);
		BaseInfo BaseInfo = new BaseInfo("20150506",sdf.format(new Date()),"803","MSG01","NJ001","00000",TransrNo,"102","RT","","1","","","");
		LCCont LCCont = new LCCont("",TransrNo,"20150506","开心保包","MSG01","888888","5","1","","","4","8","","","","","","","RMB","","","","Y","","","",lcAppnt,lcInsureds);
	...
	public static String calculate(String birthday,String sex,String amnt,String url,String brNo){
		String TransrNo = getTransrNo();
		//拼报文
		LCBnfs lcbnfs = new LCBnfs("0",null);
		Accounts acconuts = new Accounts("1");
	 ...
	 * @return
	 */
	public static TranData padding(Risk risk,LCInsured lCInsured,LCAppnt lcAppnt,LCCont lCCont,BaseInfo baseInfo){
		String TransrNo = getTransrNo();
		//补充被保险人险种信息默认值
		...
		risk.setRiskCode("HEALTHGD");
		risk.setMainRiskCode("HEALTHGD");
		risk.setBankRiskCode("017");
		risk.setRiskType("1");
		risk.setRate("");
		risk.setRank("");
		risk.setMult("");
		...
		risk.setYears("");
		risk.setSpecContent("");
		risk.setPayEndYearFlag("1");
		risk.setPayEndYear("0");
		risk.setPayoutStart("");
		risk.setPayoutEnd("");
		risk.setGetYearFlag("");
		risk.setGetYear("");
		risk.setInsuYearFlag("4");
		risk.setInsuYear("1");
		risk.setGetIntv("");
		risk.setGetBankCode("");
...
//		lCInsured.setRelaToAppnt("CH");//与投保人关系
		...
		lCInsured.setAddressStateTC("CN"); //被保人国家 CN中国
...
//		lCCont.setAccBankCode("MSG01");//保费
	...
	//老方法
	public static TranData padding(Risk risk, LCInsured lCInsured, LCAppnt lcAppnt, String flag, LCCont lCCont, String ActyNo)
	  {
	    String TransrNo = getTransrNo();
	    ...
	    lCInsured.setInsureCompanyPhone("");
	    lCInsured.setInsuredMobile("");
	    lCInsured.setHomeAddress("北京市海淀区中关村");
	    lCInsured.setHomeZipCode("310013");
	    lCInsured.setMailAddress("");
	    lCInsured.setMailZipCode("");
	    lCInsured.setEmail("");
	    lCInsured.setRelaToMain("");
	    lCInsured.setRelaToAppnt("CH");
	    lCInsured.setAppntInsuredSame("N");
	    lCInsured.setRisks(risks);
	    ...
	    lcAppnt.setAppntPhone("");
	    lcAppnt.setAppntOfficePhone("");
	    lcAppnt.setHomeZipCode("310013");
	    lcAppnt.setMailAddress("");
	    lcAppnt.setMailZipCode("");
	    lcAppnt.setJobCode("");
	    lcAppnt.setFamilyEstSalary("240000.00");
	    lcAppnt.setEstAnnualSalary("120000.00");
	    lcAppnt.setLiveZone("0");
	    lcAppnt.setRelaToInsured("PA");
	    lcAppnt.setAddressState("");
	    lcAppnt.setCity("");
	    lcAppnt.setAddressStateTC("CN");
	    
	    LCInsureds lcInsureds = new LCInsureds("1", lCInsured);
	    BaseInfo BaseInfo = new BaseInfo(df.format(new Date()), sdf.format(new Date()), "803", "MSG01", "A0000", "00000", TransrNo, flag, "RT", "", "1", "", "", "");
	    LCCont LCCont = new LCCont("", TransrNo, df.format(new Date()), "", "MSG01", "888888", "5", "1", risk.getPrem(), "", "4", "8", "", "", "", "", "", "", "RMB", "", "", "", "N",lCCont.getAccBankCode(),lCCont.getBankProvince(),lCCont.getBankCity(),lcAppnt, lcInsureds);
	    if ((lCCont.getBankAccNo() == null) || (lCCont.getBankAccNo().equals(""))) {
	      LCCont.setBankAccNo("888888");
	      LCCont.setRenewalPermit("N");
	    } else {
	      LCCont.setBankAccNo(lCCont.getBankAccNo());
	 ...
	 * @return
	 */
	public static String underwriting(String url,Risk risk,LCInsured lCInsured,LCAppnt lcAppnt,LCCont lCCont,BaseInfo baseInfo){
	...
	////老方法
	 public static String underwriting(String url, Risk risk, LCInsured lCInsured, LCAppnt lcAppnt, LCCont lCCont, String ActyNo)
	    {
	     TranData tranData = padding(risk, lCInsured, lcAppnt, "102", lCCont, ActyNo);
	     String xmlStr = beanToXML(TranData.class, tranData);
	    xmlStr = A2aOra2A(xmlStr, true);
	  
	     String responeXml = sendXml(xmlStr, url);
	     TranData respone = (TranData)XMLStringToBean(A2aOra2A(responeXml, false), TranData.class);
	    if (respone.gettranRequest().getRetData().getFlag().equals("1")) {
	       return "1";
	      }
	      return respone.gettranRequest().getRetData().getDesc();
	 ...
	 * @return flag 1成功 0失败
	 */
	public static Map<String,String> approved(String url,Risk risk,LCInsured lCInsured,LCAppnt lcAppnt,LCCont lCCont,BaseInfo baseInfo){
	...
	public static String check(ChkDetails chkDetails,String url){
		...
		baseInfo.setZoneNo("00000");//地区代码
		baseInfo.setBrNo("00000");//网点代码
...
//	@Test
//	public void 试算(){
//		System.out.println(calculate("20140112","M","500000.00","http://10.46.101.95:8080/pre/HTTPService","NJ001"));
//	}
//	
//	@Test
//	public void 承保(){
